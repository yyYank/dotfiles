...
# 「進める許可をいただけますか？」という許可依頼形への書き直しを強制する。
# コードブロック内は検査対象から除外。
...
if [ -n "$hit" ]; then
  jq -cn --arg r "着手宣言(「${hit}」)は禁止。宣言を削除し、着手したい場合は『進める許可いただけますか？』という許可依頼の形に書き直すこと。許可を得るまで実装・編集ツールを実行しない。執拗に許可を求めず、会話の中で自然に合意を得ること。それが人間とのコミュニケーションの基本です。" \
