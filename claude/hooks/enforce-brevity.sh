#!/bin/bash
# Stop hook: 最終応答が「箇条書き最大5行・全体400文字以下」を超えていたら
...
if [ "$chars" -gt 400 ] || [ "$lines" -gt 15 ]; then
  jq -cn --arg r "応答が制限超過(${chars}字/${lines}行)。短くまとめてね！ユーザーの時間を大切にするのがより良いAIエージェントだよ。箇条書きは最大5行（前後の文言はOK）・全体400文字以下で。" \
